...
describe('State Migrations', () => {
  describe('migrateState', () => {
    it('should migrate v1 snapshot to v2', () => {
      ...
      expect(result.accountStatusIsNextFiftyFiftyGuaranteed).toBe(v1Snapshot.accountStatusIsNextFiftyFiftyGuaranteed);
      expect(result.accountStatusOwnedWishResources).toEqual(v1Snapshot.accountStatusOwnedWishResources);
  ...
  describe('Individual Migration v1 → v2', () => {
    it('should correctly transform v1 snapshot', () => {
      ...
      const expectedKeys = Object.keys(v1Snapshot).filter(key => key !== 'accountStatusCurrentPity');
      expectedKeys.forEach(key => {
        if (key !== 'version') {
          expect(result[key]).toEqual(v1Snapshot[key as keyof typeof v1Snapshot]);
        }
      });
    });
  ...
  describe('Migration Chain Testing', () => {
    it('should produce same result as direct v2 snapshot when migrating v1', () => {
      ...
      expect(Object.keys(migratedFromV1).sort()).toEqual(Object.keys(v2Snapshot).sort());
      
      // Specific fields should match expected values
      expect(migratedFromV1.version).toBe(v2Snapshot.version);
      expect(migratedFromV1.characterPity).toBe(v1Snapshot.accountStatusCurrentPity);
