...
function hasRecentCommits() {
  try {
    // Check if there are any recent commits that might need changelog
...
before releasing new versions.
  ...
  if (validation.valid) {
    ...
    if (hasRecentCommits()) {
      log('yellow', '⚠️  You have recent commits that may need changelog entries.');
      log('yellow', '   Please add entries before running npm run version:patch/minor/major\n');
    }
