...
const CURRENT_SNAPSHOT_PATH = path.join(SNAPSHOTS_DIR, 'current-state-shape.json');
...
function getCurrentStateShape() {
  try {
    ...
    const result = execSync('node scripts/extract-state-shape.js', { encoding: 'utf8' });
...
function main() {
  ...
  if (command === 'check') {
    ...
    if (hasBreakingChanges(changes) && !changes.versionChanged) {
      ...
      rl.question('Automatically increment state version? (y/n): ', (answer) => {
        rl.close();
        ...
        if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
          ...
          if (result) {
            ...
            const { generateMigrationStub, updateMigrationsFile } = require('./migration-generator.js');
            const migrationStub = generateMigrationStub(result.oldVersion, result.newVersion, changes);
            
            if (updateMigrationsFile(migrationStub)) {
              console.log('✅ Version incremented and migration stub generated');
              console.log('⚠️  Don\'t forget to implement the actual migration logic!');
              
              // Re-run the check to update snapshot
              const newSnapshot = getCurrentStateShape();
              if (newSnapshot && saveSnapshot(newSnapshot, CURRENT_SNAPSHOT_PATH)) {
                console.log('📸 State snapshot updated');
              }
            }
          }
        } else {
          console.log('Please increment STATE_VERSION manually and run again');
          process.exit(1);
        }
      });
      return;
